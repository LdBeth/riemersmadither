...
long int Bottom(struct box *cube, enum dir_t dir, long int mmt[33][33][33])
...
long int Top(struct box *cube, enum dir_t dir, int pos, long int mmt[33][33][33])
...
float Maximize(struct box *cube, enum dir_t dir, int first, int last, int *cut,
...
main (int argc, char *argv[])
{
  ...
  size = cols * rows;
  Ir = malloc(size * sizeof(unsigned char));
  Ig = malloc(size * sizeof(unsigned char));
  Ib = malloc(size * sizeof(unsigned char));
  for (size_t x = 0; x < cols; x++) {
    for (size_t y = 0; y < rows; y++) {
      Ir[x * rows + y] = image[y][x].r;
      Ig[x * rows + y] = image[y][x].g;
      Ib[x * rows + y] = image[y][x].b;
    }
  }
  ...
  for (size_t x = 0; x < cols; x++) {
    for (size_t y = 0; y < rows; y++) {
      image[y][x].r = lut_r[Qadd[x * rows + y]];
      image[y][x].g = lut_g[Qadd[x * rows + y]];
      image[y][x].b = lut_b[Qadd[x * rows + y]];
    }
  }
  free(Qadd);free(tag);
